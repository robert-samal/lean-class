...
example (z : ℤ) (hz : 0 ≤ z) : ∃ n : ℕ, z = n := by
  lift z to ℕ using hz
  -- Now z : ℕ and the goal becomes ∃ n, ↑z = ↑n
  -- so we can prove it simply by ``exact ⟨z, rfl⟩``
  -- or perhaps more clearly by
  use z

example (a b c : ℕ) (h : a - b = c) (hab : b ≤ a) : (a : ℤ) - b = c := by
  zify [hab] at h
  -- Now h and the goal are stated over ℤ
  exact h
