...
-- P → Q uses the same arrow as the function below.
-- This is not by mistake, but by design -- see later.
def f : ℕ → ℝ := fun n ↦ n ^ 2 + 3
def g1 : ℕ → ℚ := fun n ↦ n ^ 2 + 3
def g2 : ℕ → ℚ := λ n ↦ n ^ 2 + 3
def g3 : ℕ → ℚ := λ n => n ^ 2 + 3
def g4 : ℕ → ℚ := fun n => n ^ 2 + 3
-- all of the variant of g give the same function, btw

-- this evaluates the anonymous function at 1, so it gives 4
#eval (fun n ↦ n^2 + 3) 1

-- this forces the output to be a rational number, written as 4/1
#eval g1 1
#eval g2 1
#eval g3 1
#eval g4 1

-- this forces the output to be a real number, namely a limit of a sequence
-- (don't worry about it though :-) )
#eval f 1

-- if we wanted to calculate with floats, we can do this as shown below.
-- However, this is not the main focus of the language.
def ff : ℕ → Float := fun n ↦ (Float.ofNat n) ^ 2 + 3
#eval ff 1
...
--  rw [tendsTo_def]
--  unfold TendsTo
-- the two lines above are equivalent to the following one:
  rw [TendsTo]
  intro ε hε
  use 100
  intro n hn
...
theorem tendsTo_add_const {a : ℕ → ℝ} {t : ℝ} (c : ℝ) (h : TendsTo a t) :
  ...
  -- use `cases` or `obtain` to deconstruct an `exists`
