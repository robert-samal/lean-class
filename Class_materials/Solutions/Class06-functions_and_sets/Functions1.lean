...
A proof has type given by what it's proved! PAT principle
-/
...
theorem A (n : ℕ) : Fintype.card (Fin n) = n := by simp
...
example (n : ℕ) : Fintype.card (Fin n) = n := Fintype.card_fin n
...
example (a b c : ℝ) : c * b * a = b * (a * c) := by
  rw [mul_comm c, mul_assoc, mul_comm a]
...
example (a b c : ℝ) : a * (b * c) = b * (c * a) := by
  rw [← mul_assoc, mul_comm a, mul_assoc, mul_comm c]
...
example (a b c d : ℝ) (hyp : c = b * a - d) (hyp' : d = a * b) : c = 0 := by
  rwa [mul_comm, ← hyp', sub_self] at hyp
...
example : (a + b) * (a + b) = a * a + 2 * (a * b) + b * b := by
   rw [mul_add, add_mul, add_mul, ← add_assoc, add_assoc (a * a), mul_comm b a, ← two_mul (a*b)]
