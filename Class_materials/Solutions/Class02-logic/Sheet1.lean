...
-- Different style
theorem my_funny_theorem (hP : P)  : P :=
  -- note that `exact P` does *not* work. `P` is the proposition, `hP` is the proof.
  hP
...
example : P → Q → P := by
  intro hP hQ
  exact hP -- alternately, we can say `assumption` (to mean goal is one of the assumptions)
  done
...
/-- alternative solution -- we are modifying our assumption, not the goal -/
example : P → (P → Q) → Q := by
  intro hP hPQ
  apply hPQ at hP
  exact hP
...
-- When the goal is split to two goals, we may want a visual separation in the proof
-- this is not mandatory, but can be done with a "." as follows:
