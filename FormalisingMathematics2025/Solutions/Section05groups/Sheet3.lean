...
-- that can be condensed a lot!
example (a b c : G) (ha : a ∈ H) (hb : b ∈ H) (hc : c ∈ H) :
    a * b⁻¹ * 1 * (a * c) ∈ H :=
  H.mul_mem (H.mul_mem (H.mul_mem ha (H.inv_mem hb)) H.one_mem) (H.mul_mem ha hc)
...
example (H K : Subgroup G) (a : G) : a ∈ H ∨ a ∈ K → a ∈ H ⊔ K := by
  intro h
  cases h with
  | inl h => exact Subgroup.mem_sup_left h
  | inr h => exact Subgroup.mem_sup_right h
